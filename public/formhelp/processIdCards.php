...
if (!empty($_POST['idName']) && !empty($_FILES['idPhoto'])) {
    ...
    if ($error === 0) {
        if ($img_size > 125000) {
            header("Location:../idCards.php?error=size%20is%20too%20large&email=$email");
        } else {
            ...
            if (in_array($img_ex_lc, $allowed_exs)) {
                ...
                header("Location:../idCards.php?error=Information%20Added%20Successfully&email=$email");
            } else {
                header("Location:../idCards.php?error=Image%20Type%20is%20not%20Convinient&email=$email");
            }
        }
    } else {
        //unknown error
        header("Location:../idCards.php?error=Unknown%20Error%20Occured&email=$email");
    }
}
